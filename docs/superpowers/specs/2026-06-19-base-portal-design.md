...
当前 tab 支持应用内最大化，不调用浏览器 Fullscreen API。最大化时隐藏顶部、左侧和 tab，让功能页内容占满浏览器窗口，并保留退出最大化按钮。
...
首版不做 CSP、网络、登录状态等复杂诊断。

### 响应式

桌面和平板优先。手机端做兜底，Portal 壳不崩；嵌入页面自身适配由第三方应用负责。
