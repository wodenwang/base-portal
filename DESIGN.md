...
Base Portal 是企业内部第三方应用门户壳，用于承载多个独立系统。它的核心职责是登录后提供功能域切换、最多三层菜单、Tab 工作区、iframe 承载、可恢复打开方式和应用内最大化。
...
v0.3.0 设计决策见 `design/2026-06-20-v0.3.0-design-decision.md`：移除沉浸模式，保留标准 iframe 和应用内最大化；历史 `immersive_iframe` 数据运行时兼容降级为标准 iframe；Tab 右键菜单和 Tab 操作菜单都提供刷新当前 iframe 的恢复动作。
...
- iframe 标准模式保留完整 Portal 壳；最大化临时覆盖整个 Portal 壳。
- 最大化退出后必须回到标准 iframe 工作区，不能让用户迷失。
- 历史 `immersive_iframe` 打开方式不再作为当前产品能力呈现，运行时应兼容降级为标准 iframe。
...
- 历史或配置中的 `new_tab` 打开方式只作为打开偏好 hint；菜单点击默认仍打开或激活 Portal 内部业务 tab，新窗口打开由 tab 右键菜单、Tab 操作菜单或 iframe fallback 提供。
- iframe fallback 文案只能表达“页面可能无法加载”，不能声称准确识别 CSP、网络、登录态或跨域原因。
- 复制链接、新窗口打开、刷新等恢复动作必须明显且不遮挡 iframe 的基础可见性。
- 当前 iframe 的刷新动作必须只重载当前业务 tab 的 iframe，不刷新整个 Portal，不改变 tab 顺序或激活状态。
- 桌面端业务 tab 右键菜单必须提供刷新当前 iframe；平板和手机端 Tab 操作菜单必须提供同等能力。
