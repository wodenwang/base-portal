...
- `config/portal-apps/base-portal-demo.json`：v0.2.0 第三方应用接入包示例，不包含任何 secret。
- `deploy/docker-compose.yml`：Docker Compose 运行环境，包含 PostgreSQL 和 Web/API 服务。
- `DEPLOY.md`：v0.2.0 部署和升级治理，覆盖 Docker Compose、安装、升级、DNS、证书和 nginx 反向代理。
...
v0.2.0 不新增配置 UI。第三方应用接入先使用文件导入 + ops API：
...
生产部署必须先读取并遵守 [DEPLOY.md](DEPLOY.md)。v0.2.0 生产目标为：
...
首次安装使用 `install.sh`，版本升级使用 `upgrade.sh`。真实 `deploy/.env`、IAM secret、阿里云凭证和证书私钥不得提交或回显。v0.2.0 发布后还必须读回 `/version` 为 `0.2.0`，并保存真实 IAM 登录、权限读取、导入 dry-run/apply、IAM sync dry-run/apply 的脱敏证据。
